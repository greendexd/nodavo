<!-- doc-id: readme; lang: ru; translation-of: README.md; revision: 19 -->
...
| Обновления | Подписанные проверки, явное согласие, безопасная установка и откат | В агенте есть выключенный по умолчанию, не активирующий обновления slice с проверкой подписанного манифеста, согласием на точный offer, проверяемым staging macOS и двуязычным status macOS. В исходниках также есть ограниченный reducer внешнего supervisor, проверка sealed macOS bundle и приватный Windows staging/read-only Appx inspection. Production endpoint и signing key не использовались; отсутствуют supervisor executable, установка, активация, restart/rollback handoff, защищённое состояние обновлений, Windows UI и физическая квалификация обновления |
...
- Не активирующий обновления slice в агенте и Settings macOS, а также source-only foundations для отдельно подписанного supervisor. Ограниченный reducer требует отдельного согласия на установку и перезапуск, точного evidence candidate/predecessor, аутентифицированного завершения process attempt, journal-before-effect и продвижения floor до удаления backup. macOS умеет проверять и удерживать точное sealed universal app tree, но намеренно не предоставляет swap API; Windows добавляет приватный fixed-volume staging и ограниченный read-only Appx inspection. Packaging pipeline создаёт sealed universal update ZIP и точные metadata hash/size; фактически собран только его явно development-labeled вариант. Ничто здесь не устанавливает и не выполняет staged content.
...
- Реальная проверка peer file transfer, выбираемые пользователем receive destinations, durable progress/history и peer/outbound ownership между перезапусками агента, display layout editor и физическая квалификация hot-plug, onboarding, экспортируемые diagnostics, а также отдельно упакованный updater supervisor с защищёнными journals, platform activation, restart/health/rollback handoff, Windows UI и физической power-loss квалификацией.
