<!-- doc-id: readme; lang: ru; translation-of: README.md; revision: 23 -->
...
| Переход через край экрана | Разный масштаб DPI и несколько мониторов | Обе native shells теперь дают для каждого доверенного peer сохраняемое положение из пяти состояний: отключено, слева, справа, сверху или снизу. Agent выводит только временные bounded exterior edge routes из текущих committed topologies, input grant peer и точного acknowledgement любой локальной revision, directionally опубликованной для inbound control; outbound-only локальный graph намеренно не публикуется. Native/session display identifiers не сохраняются. Смена положения при non-local focus сначала восстанавливает локальное владение и закрывает эту session. Подключены authenticated hot-plug refresh, mixed-DPI transforms, debounce/hysteresis, reliable entry и relative deltas. Всё ещё нет per-display drag/partial-edge editor и физической cross-platform проверки hot-plug/multi-monitor |
...
- Двуязычная SwiftUI-оболочка macOS в строке меню, подключённая к агенту через release signed XPC (или явный unsafe development UDS), с pairing, управлением trusted devices/grants, подтверждённым revocation, per-peer placement, manual focus, явным выбором файлов/папок, ограниченными progress/cancellation без имён файлов и карточкой готовности. Placement mutation использует точный acknowledgement или status-only reconciliation и никогда не отправляется повторно вслепую. Действие разрешения запрашивает Accessibility от identity агента и применяет только свежий probe; показ системного prompt никогда не считается выдачей доступа.
- Двуязычная WinUI 3-оболочка со строгим ограниченным status, раздельными сигналами готовности, emergency stop, one-shot manual focus, per-peer placement, pairing, управлением trusted devices/grants, подтверждённым revocation, явным выбором файлов/папок, ограниченными progress/cancellation без имён файлов, fixed package-root agent launch action и opt-in запуском при входе. Неоднозначный результат focus, placement, grant или transfer запускает reconciliation или блокирует retry, а не изображает rollback и не повторяет mutation. До любой команды shell проверяет собственный package root и точный signed agent process/image внутри него; agent взаимно проверяет настроенный packaged UI и закрывает connection после одного response. GitHub Actions компилирует Release x64, а workflow упаковки теперь проверяет установку и inspection точного self-signed development MSIX и ограниченный self-check точного установленного agent. Этот headless gate не выполняет установленные интерактивные пути lifecycle, readiness, focus, placement, transfer, `Package.Current` и взаимной pipe authentication.
- Оригинальные macOS/Windows input и clipboard runtimes, подключённые к agent, с default-off suppression, injected-event rejection, bounded relative motion, транзакционным display hot-plug refresh, lifecycle recovery, deterministic release и content-redacted failures. Hot-plug callbacks только сигнализируют об изменении; до возобновления focus выполняются stable full snapshot, callback/admission barriers, drain очереди и освобождение lease. Replacement local revision также требует точного peer acknowledgement, когда эта topology directionally публикуется для inbound control.
- Authenticated bounded file channels, process worker с четырьмя reservations, cooperative scan cancellation, peer-scoped cleanup, private capability-rooted receiver staging и outbound filesystem source с exact hashes, mutation detection, no-follow handles, Windows birth-time DACL и консервативной no-overwrite publication. Release receiver публикует только через opaque retained handle фиксированного OS-known каталога `Загрузки/Nodavo`; ошибка destination отклоняет новое входящее право Files до сохранения trust, а input-only sessions остаются рабочими. Отдельный short-held registry выдаёт process-local public IDs, ограничивает live/recent rows, продвигает outbound bytes только после reliable send, inbound bytes — только после durable staging write, и линеаризует targeted cancellation с finalization.
- Universal development app/DMG pipeline для macOS и fail-closed release path Developer ID/provisioning/notarization. Локальная и GitHub development-упаковка запускают точный packaged SwiftUI binary в development-only passive mode против встроенного helper через изолированный UDS; он выполняет ровно по одному строгому read-only вызову status, focus-status, списка trusted peers и списка transfers. XPC, signing, TCC, input, peers и hardware при этом не проверяются. Release credentials недоступны, поэтому собран только явно непригодный для распространения development artifact.
...
- Реальная проверка peer file transfer, выбираемые пользователем receive destinations, durable progress/history и peer/outbound ownership между перезапусками агента, per-display drag/partial-edge layout editor и физическая квалификация hot-plug, onboarding, экспортируемые diagnostics, а также отдельно упакованный updater supervisor с mutually authenticated IPC, protected journals, platform activation, process adapters для restart/health/rollback, Windows UI и физической power-loss квалификацией.
