<!-- doc-id: security-model; lang: ru; translation-of: security-model.md; revision: 20 -->
...
- Native display callback служит только coalesced dirty-сигналом. Platform публикует замену лишь после стабилизации ограниченного полного graph; retired native identities никогда не переиспользуются. macOS использует CoreGraphics reconfiguration observation, а Windows сочетает broadcast-capable hidden window с authoritative full polling и process-wide per-monitor-v2 DPI awareness.
...
- Готовность session topology независима от обнаружения локальных дисплеев. Она становится ready только после установки точной аутентифицированной remote topology и либо точного acknowledgement локальной revision, опубликованной для inbound control, либо намеренно unpublished local graph при отсутствии inbound input grant. При disconnect, recovery, revoke или failure состояние сбрасывается.
- Изменение локальных дисплеев немедленно блокирует новый focus. Агент закрывает native routing admission, ждёт in-flight suppression decisions, дренирует ограниченную очередь уже принятых input под старой lease, выполняет forced releases, восстанавливает local ownership и устанавливает только последнюю проверенную map. Если local topology directionally публикуется для inbound control, focus не возобновляется до acknowledgement этой точной replacement revision; outbound-only operation использует тот же stable committed snapshot без его публикации. Snapshot acquisition, sends и acknowledgement имеют фиксированные deadlines, которые hot-plug storm не продлевает.
- Аутентифицированный input для любой неактуальной lease инертен до продвижения sequence или разрешения native display. Absolute pointer injection — единственный topology-sensitive retry: он удерживается в capacity one и повторяется не более одного раза; keys, buttons, scroll, relative deltas и forced releases не зависят от display geometry.
- Teardown capture и injector ограничен deadline. Timeout или fatal failure native owner навсегда poison’ит этот process-local runtime, поэтому detached old worker не может пересечься с новой session; failed release или ownership restore latch’ит shared safety state до любого последующего close или readiness publication.
...
- Topology revision должна быть установлена и, если она directionally опубликована, подтверждена до переноса focus в соответствующем направлении. Persistent placement содержит только peer identifier и одно из значений disabled/left/right/above/below; native и session display identifiers в нём не представимы. Ephemeral edge routes выводятся только из актуальных committed topologies, peer input grant и точного acknowledgement любой локальной revision, опубликованной для inbound control; outbound-only локальный graph остаётся unpublished. Routes отключены при пустом или stale состоянии и всё равно проходят обычные проверки focus lease, reliable pointer-entry acknowledgement, debounce, hysteresis и cooldown. Смена placement при non-local focus выполняет safety recovery и disconnect до повторного использования сохранённой policy.
...
Release receive authority — это фиксированный каталог `Загрузки/Nodavo`, а не путь из IPC или сети. macOS разрешает пользовательские «Загрузки» через `NSFileManager` и отклоняет любой extended ACL на финальном каталоге; Windows использует `SHGetKnownFolderPath(FOLDERID_Downloads)` для текущего пользователя и отклоняет нефиксированные volumes. Обе platform boundaries отклоняют unsafe namespace components и reparse points, создают или проверяют точный owner-private leaf и передают generic receiver уже открытый no-follow directory handle. Receiver независимо повторно проверяет owner/privacy policy, забывает ambient path и удерживает только capability для publication, revocation и offline cleanup. Отсутствующий или запрещённый destination отклоняет входящий Files grant при pairing/изменении прав до сохранения trust record или grant epoch; он не блокирует input-only peer. Существующее скрытое pre-alpha receive state не мигрируется и не удаляется. Абсолютные receive paths не попадают в public status, errors, transfer snapshots или logs, а полученный контент никогда не открывается автоматически.
...
- Listing доверенных устройств ограничен 32 публичными summaries, содержащими только локальный peer identifier, bounded display name, состояние active/revoked, локально выданные grants и семантическое placement из пяти состояний. Сертификаты, endpoints, native/session display identifiers, private material и независимо выданные peer grants исключены. Placement и manual-focus mutations являются one-shot: неоднозначный response разрешает только status reconciliation, а не слепую повторную отправку. Malformed status или error envelope нельзя считать детерминированным acknowledgement или rejection.
...
Принятые артефакты возобновляемо загружаются в приватный capability-rooted content-addressed staging под межпроцессной exclusive lease и ограничениями quota/retention. Запись проходит потоковые проверки точного offset, размера и digest; файлы и поддерживаемые изменения каталогов синхронизируются до сообщения о verified staging. Двуязычный раздел Settings macOS умеет вручную проверять или обновлять статус, опрашивать ограниченный progress через отдельный client, показывать результат up-to-date, принимать или отклонять точное предложение и возобновлять приостановленную загрузку, не отображая endpoint, filesystem path или digest.

Source-only reducer внешнего supervision является policy boundary, а не installer. Отдельное одноразовое решение **Установить и перезапустить** может преобразовать только точную `ReadyToInstall` session в ограниченный handoff с ненулевым request ID и точным исходным signed manifest envelope. В default-сборке agent этот type предоставляет encoding и request ID, но не decoding, доступ к raw envelope, методы supervisor admission/host, reducer state или actions. Repository и packaging gates отклоняют non-default feature `supervisor-host` в agent artifacts. Это исключение сокращает случайную authority заменяемого process, но не является runtime authentication: будущий supervisor всё равно обязан взаимно аутентифицировать old process и точные installed target/version, удерживать exclusive lock и использовать protected authenticated state.

Supervisor-side contract предварительно резервирует request без его durable consumption, загружает свежий rollback floor, повторно проверяет точный исходный envelope по supervisor-local policy, локально выводит plan и заново открывает и хэширует точный sealed content-addressed artifact. Только после этого он может атомарно сохранить replay tombstone, exact old-process binding и schema 3 journal, связывающий request ID, сгенерированные supervisor identities transaction и attempt, evidence candidate/predecessor и rollback state. Ошибки до persistence ничего не разрешают. После вызова persistence ошибка, malformed authenticated reload или неточный reload означают outcome unknown: lock и admission остаются закрытыми, а любой retry, reducer action или новый request может разрешить только recovery из аутентифицированного хранилища. Затем reducer требует journal-before-effect transitions, ограниченных process attempts, аутентифицированного exact-process exit после timeout, health candidate до commit и продвижения floor до удаления backup. Shutdown path агента выполняет bounded fail-closed drain и оставляет admission session/transfer закрытым, но update IPC не запускает supervisor.

Исходники macOS умеют проверять и удерживать ограниченное sealed universal app tree с точными signing, entitlement, notarization/System Policy, ownership, mode, ACL, hardlink, content и mutation proofs. Они намеренно не предоставляют swap, activation или rollback primitive: проверенный unprivileged cross-parent exchange несовместим с immutable состоянием обоих trees. Windows source добавляет owner-only fixed-volume content-addressed staging, межпроцессную lease, quota/retention и ограниченный read-only Appx inspection из retained handles. Он не выполняет package deployment, registration, activation, launch или rollback, а native behavior ещё требует Windows runtime qualification.

Репозиторий не содержит production endpoint, приватного release signing key или доказательства live production update check. В нём также нет защищённого production update journal или durable rollback floor, отдельно упакованного supervisor executable, authenticated supervisor IPC, activation adapter, process wiring для restart/health/rollback и физического power-loss proof. Staged content никогда не исполняется и не устанавливается. Это release gates, а не свойства текущих source foundations.
