<!-- doc-id: security-model; lang: ru; translation-of: security-model.md; revision: 17 -->
...
Source-only reducer внешнего supervision является policy boundary, а не installer. Он требует отдельного одноразового согласия **Установить и перезапустить**, exclusive transaction lock, свежего rollback-floor state, точного evidence candidate и сохранённого predecessor, journal-before-effect transitions, ограниченных process attempts, аутентифицированного exact-process exit после timeout, health candidate до commit и продвижения floor до удаления backup. Старые in-process activation helpers доступны только тестам. Shutdown path агента выполняет bounded fail-closed drain и оставляет admission session/transfer закрытым, но update IPC не запускает supervisor.
