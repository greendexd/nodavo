<!-- doc-id: security-model; lang: ru; translation-of: security-model.md; revision: 18 -->
...
- Изменение локальных дисплеев немедленно блокирует новый focus. Агент закрывает native routing admission, ждёт in-flight suppression decisions, дренирует ограниченную очередь уже принятых input под старой lease, выполняет forced releases и восстанавливает local ownership, устанавливает только последнюю проверенную map и публикует одну replacement revision. Focus не возобновляется до acknowledgement этой точной revision. Snapshot acquisition, sends и acknowledgement имеют фиксированные deadlines, которые hot-plug storm не продлевает.
