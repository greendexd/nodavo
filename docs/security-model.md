<!-- doc-id: security-model; lang: en; revision: 17 -->
...
The source-only external-supervision reducer is a policy boundary, not an installer. It requires separate one-shot **Install and restart** consent, an exclusive transaction lock, fresh rollback-floor state, exact candidate and retained-predecessor evidence, journal-before-effect transitions, bounded process attempts, authenticated exact-process exit after timeout, candidate health before commit, and floor advancement before backup retirement. Legacy in-process activation helpers are test-only. The agent's shutdown path performs a bounded fail-closed drain and leaves session/transfer admission closed, but no update IPC invokes a supervisor.
...
This repository supplies no production endpoint, release private signing key, or evidence of a live production update check. It also has no protected production update journal or durable rollback floor, separately packaged supervisor, activation adapter, restart/health IPC, or physical power-loss proof. Staged content is never executed or installed. These are release gates rather than properties of the current source foundations.
