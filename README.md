<!-- doc-id: readme; lang: en; revision: 23 -->
...
| Seamless edge switching | Mixed DPI and multi-monitor layouts | Both native shells now expose a persisted five-state placement for each trusted peer: disabled, left, right, above, or below. The agent derives only ephemeral bounded exterior edge routes from current committed topologies, the peer's input grant, and the exact acknowledgement of any local revision directionally published for inbound control; an outbound-only local graph is intentionally not published. It never persists native or session display identifiers. A placement change during non-local focus first restores local ownership and closes that session. Authenticated hot-plug refresh, mixed-DPI transforms, debounce/hysteresis, reliable entry, and relative deltas are wired. A per-display drag/partial-edge editor and physical cross-platform hot-plug/multi-monitor proof are still missing |
...
| Updates | Signed checks, explicit consent, safe install and rollback | The agent has an unconfigured-by-default, non-activating update slice with signed-manifest checks, exact-offer consent, verified macOS staging, and bilingual macOS status. A staged session with distinct install-and-restart consent can now be consumed into a bounded encode-only supervisor handoff that retains the exact signed envelope; reducer admission, decoding, and actions are excluded from the ordinary agent build. Source also contains sealed macOS bundle validation and private Windows staging/read-only Appx inspection. No production endpoint or signing key has been used; no supervisor executable or IPC, protected update-state persistence, installation, activation, restart/health/rollback process wiring, Windows UI, or physical update qualification exists |
| Platforms | macOS 13+ and Windows 10 22H2/11, x64 and ARM64 | SwiftUI and WinUI 3 x64 compile in CI; Rust checks for macOS arm64/x64 and Windows x64. Both shells strictly decode content-free readiness for agent reachability, local input prerequisites, local displays, and authenticated peer-topology synchronization; macOS can ask the agent to show the Accessibility prompt and then rechecks actual trust. The readiness probe never creates a second native capture registration, so `ready` is an honest prerequisite signal rather than live capture proof. Correctly signed macOS release builds use reciprocal per-message XPC code requirements over a fixed LaunchAgent Mach service with no UDS fallback; development packaging alone compiles an explicit unsafe same-UID UDS bypass. That development path now runs the exact packaged SwiftUI binary through a development-only passive smoke against its embedded helper over an isolated UDS, making exactly four strict read-only calls. Windows source reciprocally binds each one-request named-pipe connection to the exact packaged UI and exact signed agent image beneath that UI package's installed root, and exposes a fixed launch action plus an opt-in StartupTask. The Windows packaging workflow now gates installation of the exact self-signed development MSIX, validation of its identity, manifest, and disabled StartupTask, a self-check of the exact installed agent through the packaged desktop command API, and removal of the package and temporary certificate. These gates do not prove signed XPC, TCC, an interactive Windows UI or `Package.Current` launch, mutual pipe/status authentication, startup enablement, a peer session, physical hardware, ARM64 execution, release signing, or clean installer matrices |
...
- A bilingual SwiftUI macOS menu-bar shell connected to the local agent through release signed XPC (or the explicit unsafe development UDS), including pairing, trusted-device/grant management, confirmed revocation, per-peer placement, manual focus, explicit file/folder selection, bounded filename-free transfer progress/cancellation, and a readiness card. Placement mutations use exact acknowledgements or status-only reconciliation and are never blindly resent. The permission action asks through the agent identity and applies only a fresh probe; showing the system prompt is never treated as a grant.
- A bilingual WinUI 3 shell with bounded strict status, separate readiness signals, emergency stop, one-shot manual focus, per-peer placement, pairing, trusted-device/grant management, confirmed revocation, explicit file/folder selection, bounded filename-free transfer progress/cancellation, a fixed package-root agent launch action, and opt-in login startup. Ambiguous focus, placement, grant, and transfer outcomes reconcile or lock retries instead of claiming rollback or resending a mutation. Before any command, the shell verifies its own package root and the exact signed agent process/image inside it; the agent reciprocally verifies the configured packaged UI and closes after one response. GitHub Actions compiles its x64 Release configuration, and the packaging workflow now gates installation and inspection of the exact self-signed development MSIX plus a bounded self-check of its exact installed agent. The installed interactive lifecycle, readiness, focus, placement, transfer, `Package.Current`, and mutual-pipe-authentication paths have not been exercised by that headless gate.
- Original macOS and Windows input/clipboard runtimes wired into the agent, with default-off suppression, injected-event rejection, bounded relative motion, transactional display hot-plug refresh, lifecycle recovery, deterministic release, and content-redacted failures. Hot-plug callbacks only signal change; stable full snapshots, callback/admission barriers, queue draining, and lease quiescence run before focus resumes. A replacement local revision also requires an exact peer acknowledgement whenever that topology is directionally published for inbound control.
- Authenticated bounded file channels, a four-reservation process worker, cooperative scan cancellation, peer-scoped cleanup, private capability-rooted receiver staging, and an outbound filesystem source with exact hashes, mutation detection, no-follow handles, Windows birth-time DACLs, and conservative no-overwrite publication. Release receivers publish only through an opaque retained handle for the fixed OS-known `Downloads/Nodavo` leaf; destination failure rejects a new inbound Files grant before trust persistence while input-only sessions remain usable. A separate short-held registry assigns process-local public IDs, caps live/recent rows, advances outbound bytes only after reliable sends and inbound bytes only after durable staging writes, and linearizes targeted cancellation against finalization.
- A universal development macOS app/DMG pipeline and a fail-closed Developer ID/provisioning/notarization release path. Local and GitHub development packaging run the exact packaged SwiftUI binary in a development-only passive mode against the embedded helper over an isolated UDS; it makes exactly one strict read-only status, focus-status, trusted-peer-list, and transfer-list call. It does not exercise XPC, signing, TCC, input, peers, or hardware. Release credentials are unavailable, so only the explicitly non-distributable development artifact has been built.
- A non-activating signed-update slice in the agent and macOS Settings, plus source-only foundations for a separately signed supervisor. Only a `ReadyToInstall` session with distinct install-and-restart consent can be consumed into a bounded one-shot handoff; the default agent can encode that request but cannot decode it or link reducer admission/actions. The handoff retains the exact originally verified signed envelope for authoritative supervisor re-verification, and supervision journal schema 3 binds its nonzero request ID. The package gates reject the non-default `supervisor-host` feature from ordinary agent artifacts; this build boundary does not replace mutual process authentication, protected storage, or exclusive locking. macOS can validate and retain an exact sealed universal app tree but intentionally exposes no swap API; Windows adds private fixed-volume staging and bounded read-only Appx inspection. The package pipeline emits a sealed universal update ZIP and exact hash/size metadata; only its explicitly development-labeled variant has been produced. Nothing here installs or executes staged content.
...
- Real peer file-transfer qualification, user-selected receive destinations, durable progress/history and peer/outbound ownership across agent restart, a per-display drag/partial-edge layout editor and physical hot-plug qualification, onboarding, exportable diagnostics, and a separately packaged updater supervisor with mutually authenticated IPC, protected journals, platform activation, restart/health/rollback process adapters, Windows UI, and physical power-loss qualification.
